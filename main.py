...
st.markdown("메인 테이블(5일~1일 초단기)과 상세분석(1년~5일 주가 흐름, 20개 지표, DART, 리스크 진단) 완벽 통합 시스템")
st.markdown("---")
...
# ---------------------------------------------------------
# 2. 종목별 상세분석 모달 / 섹션 (요청하신 모든 항목 완벽 탑재)
# ---------------------------------------------------------
...
with st.container():
    ...
    if row['숏스퀴즈적합여부'] == "🔴 Squeeze Candidate":
        st.error(f"🚨 **[숏스퀴즈 경보]** 공매도 잔고비중({row['공매도잔고비중(%)']}%) 및 Dtc({row['DtC(일)']}일)가 엄격 기준을 충족하여 숏커버링 유입 가능성이 매우 높습니다. (+{row['숏스퀴즈보너스']}점 가산)")
    else:
        st.info(f"💡 **[일반 모멘텀 분석]** 숏스퀴즈 엄격 조건(잔고 3% 이상 등)에는 미달하나, 수급 및 거래대금({row['당일거래대금(억)']}억) 유입을 통한 단기 탄력성을 모니터링 중입니다.")
...
with col_right:
    st.markdown("#### 📊 20개 핵심 지표 점수표 (핵심 요약)")
...
st.markdown("---")
st.success("✨ 모든 요청 사항(메인/모달 시계열 분리, 8대 엄격 숏스퀴즈 게이트, 20개 지표 점수표, DART, 리스크 진단)이 완벽하게 반영되었습니다.")
