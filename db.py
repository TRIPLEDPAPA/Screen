...
def init_db():
    with get_connection() as conn:
        ...
        # 테이블 초기화 및 과거 기준가/상세 JSON 컬럼 보장
        cursor.execute("""
        ...
        # 기존 DB 테이블 마이그레이션 대비 ALTER 컬럼 자동 추가
    ...
    with get_connection() as conn:
        ...
        for item in candidates:
            ...
            INSERT INTO daily_candidates (
                ...
                foreign_inst_net, ref_5d, ref_1m, ref_3m, ref_6m, ref_1y, detail_json, updated_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
def get_all_candidates() -> list[dict[str, Any]]:
    with get_connection() as conn:
        ...
        for row in rows:
            ...
            r1m = round(((cur_p - (r["ref_1m"] or cur_p)) / (r["ref_1m"] or cur_p)) * 100, 1)
